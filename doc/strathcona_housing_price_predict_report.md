...
A Ridge Regression approach was used to model property assessment values against our set of available features. The features were classified in three types in order to conduct distinct transformations: categorical features, binary features and continuous features. The alpha hyperparameter for the Ridge Regression was tuned using cross-validation over the discrete set of alpha values of 0.0001, 0.001, 0.01, and up to 1000. Assessment of the model, based on training data only, was made with cross-validation (using 10-folds) and was scored using $R^2$. 
